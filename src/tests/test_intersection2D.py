...
def deformable_piecewise(device, workingDirectory, segment,voxelToMmRatio,simCoef=1,colCoef=1,dilation=None):
    ...
    for scale in scales:
        ...
        integrator = numericalIntegration.trapezoidalNumInt(10, corrector_steps=10, stationary=False)
...
def main():
    ...
    path=os.path.split(current_dir)[0]
    workingDirectory = os.path.join(path,"syntheticData/intersection2D")
    device = "cuda:0"
